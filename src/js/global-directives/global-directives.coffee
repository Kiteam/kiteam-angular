...
define [
  ...
  '../utils'
  '../vendor/lodash'
  './views/global-all.html'
  '../project/views/project-all.html'
  '../vendor/keyboard'
  'imports?jQuery=jquery!../plugin/jquery.honey.simple-tab'
  '../../package/datetime/datetimepicker'
  ...
  .directive('instantSearch', ['$rootScope', ($rootScope)->
    ...
    replace: true
    link: (scope, element, attrs)->
      element.bind 'focus', (event)-> element.css(width: '100%', opacity: 1)

      element.bind 'blur', (event)-> element.css(width: '80%', opacity: 0.8)
